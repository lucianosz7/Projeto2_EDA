...
struct hashnode *create_hashnode(char *palavra){
...
struct hash *insert_value(struct hash *htable,char *palavra, int hsize)
{
if(htable==NULL){
	htable = create_hash(MAX);
}
int slot;
...
slot = hashcal(palavra)/50000000;

if (htptr[slot].ptr == NULL) {

tmp = create_hashnode(palavra);
...
int search_hashtable(struct hash *htable,const char *palavra, int size){
int slot, found = 0;
slot = hashcal(palavra)/50000000;
struct hashnode *tmp;
tmp = htable[slot].ptr;
while (tmp->next != NULL) {
if (strcmp(tmp->palavra,palavra)==1) {
found = 1;
break;
}
tmp = tmp->next;
...
bool conferePalavra(const char *palavra) {
    if(search_hashtable(hashtables,palavra,MAX)==1){
      return true;
    }
    else{
    return false;
  ...
  if(dicionario!=NULL){
    ...
    for (c = fgetc(fd); c != EOF; c = fgetc(fd)) {
        ...
        /* encontra uma palavra completa */
            ...
            palavra[indice] = '\0';
            hashtables = insert_value(hashtables,palavra,MAX);
            top++;
            i++;
            indice = 0;
        } /* fim-if */
