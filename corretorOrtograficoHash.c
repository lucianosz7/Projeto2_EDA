...
unsigned long long int hashtable[MAX];
int top=0;

int hash(const char *palavra){
		int i,h = palavra[0];
		for(i=1;palavra[i]!='\0';i++){
			h = (h * 251 + palavra[i])%5000000;
		}
		return h;
	}
...
bool conferePalavra(const char *palavra) {
    if(hashtable[hash(palavra)%10000000]== 1){
      return true;
    }
    else{
    return false;
  ...
  if(dicionario!=NULL){
    ...
    for (c = fgetc(fd); c != EOF; c = fgetc(fd)) {
        ...
        /* encontra uma palavra completa */
            ...
            palavra[indice] = '\0';
            hashtable[hash(palavra)%10000000]=1;
            top++;
            i++;
            indice = 0;
        } /* fim-if */
